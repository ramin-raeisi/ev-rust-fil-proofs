    ...
    R: Read,
{
    ...
    fn read_to_boundary(&mut self) -> io::Result<&[u8]> {
        ...
        trace!("Before search Buf len: {} Search idx: {} State: {:?}",
        ...
        //trace!("After search Buf len: {} Search idx: {} State: {:?}",
        //       buf.len(), self.search_idx, self.state);
    ...
    pub fn consume_boundary(&mut self) -> io::Result<bool> {
        ...
        while self.state == Searching {
            ...
            trace!("Discarding {} bytes", buf_len);
            ...
            match last_two {
                ...
                _ => {
                    return Err(io::Error::new(
                        ...
                        format!(
                            "unexpected bytes following multipart boundary: {:X} {:X}",
...
impl<R> BufRead for BoundaryReader<R>
where
    R: Read,
{
    ...
    fn consume(&mut self, amt: usize) {
        ...
        //trace!("Consume! amt: {} true amt: {}", amt, true_amt);
...
mod test {
    ...
    fn test_boundary() {
        ...
        //trace!("Testing boundary (no split)");
    ...
    fn test_unterminated_body() {
        ...
        //trace!("Consume 3 - expecting error");
