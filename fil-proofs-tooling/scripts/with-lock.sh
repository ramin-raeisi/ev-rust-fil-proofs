...
for lockdir_pid in $(find "$lockdir" -type f -exec basename {} \; 2>/dev/null); do
    if ! ps -p "${lockdir_pid}" >/dev/null; then
        (echo >&2 "cleaning up leaked lock (pid=${lockdir_pid}, path=${lockdir})")
        rm -rf "${lockdir}"
    fi
done

if mkdir "$lockdir" >/dev/null 2>&1; then
    (echo >&2 "successfully acquired lock (pid=$$, path=${lockdir})")
    ...
    (echo >&2 "failed to acquire lock (path=${lockdir})")
    exit "$failure_code"
fi
