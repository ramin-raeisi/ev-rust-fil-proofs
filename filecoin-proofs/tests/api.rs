...
fn create_seal<R: Rng, Tree: 'static + MerkleTreeTrait>(
    ...
    let pre_commit_output = seal_pre_commit_phase2(
        ...
        phase1_output,
        cache_dir.path(),
        sealed_sector_file.path(),  //replica_path
    )?;
...
#[test]
//TODO-Ryan: UT P1, P2, C1 
fn test_seal_simple_2kib_base_8() -> Result<()> {
    // assert!(false, "test -- failed -- test");
    seal_simple_lifecycle::<SectorShape2KiB>(SECTOR_SIZE_2_KIB)
    // seal_simple_lifecycle::<SectorShape8MiB>(SECTOR_SIZE_8_MIB)
}

fn seal_simple_lifecycle<Tree: 'static + MerkleTreeTrait>(sector_size: u64) -> Result<()> {
    let rng = &mut XorShiftRng::from_seed(TEST_SEED);
    let prover_fr: DefaultTreeDomain = Fr::random(rng).into();
    let mut prover_id = [0u8; 32];
    prover_id.copy_from_slice(AsRef::<[u8]>::as_ref(&prover_fr));

    create_seal_simple::<_, Tree>(rng, sector_size, prover_id, false)?;
    Ok(())
}

fn create_seal_simple<R: Rng, Tree: 'static + MerkleTreeTrait>(
    rng: &mut R,
    sector_size: u64,
    prover_id: ProverId,
    skip_proof: bool,
    ...
    // let comm_d = pre_commit_output.comm_d;  // from seal_pre_commit_phase2
    let comm_r = pre_commit_output.comm_r;
