...
use sds::service::storage::download::{sds_is_enable, reader_from_env, RangeReader};
...
fn read_aux<P: AsRef<Path>>(path: P) -> std::io::Result<Vec<u8>> {
    ...
    if Path::new((&path).as_ref().as_os_str()).exists() || !sds_enable || r.is_none() {
...
pub fn generate_winning_post<Tree: 'static + MerkleTreeTrait>(
    ...
    info!("winning init tree {} sds-time {:?}", trees.len(), t2.elapsed());
...
pub fn generate_window_post<Tree: 'static + MerkleTreeTrait>(
    ...
    let trees: Vec<_> = replicas
        ...
        .collect::<Result<_>>()?;
    info!("window init tree {} sds-time {:?}", trees.len(), t2.elapsed());
